...
센서 채널이 **침묵(silent)** 하거나 **고착(stuck)** 되면, 사람이 손대기 전에
가장 안전한 조치인 **채널 재시작**을 자동으로 시도한다(기본 켜짐).

| 환경변수 | 기본값 | 설명 |
|---|---|---|
| `JCC_AUTOHEAL` | `1` | `0`이면 자동복구를 끈다. |
| `JCC_HEAL_MAX_RETRY` | `2` | 한 장애당 최대 재시작 횟수. 넘으면 포기하고 사람에게 넘김(→ 상향·알림). |
| `JCC_HEAL_COOLDOWN` | `60` | 같은 센서 재시도 간격(초). |
| `JCC_HEAL_DAILY_CAP` | `30` | 하루 자동 재시작 총량. 넘으면 그날은 자동개입 중단. |

- 드리프트·이상(보정/공정 문제)과 CCM 전체 침묵은 자동 재시작 대상이 아니다(각각 L2·사람 판단 영역).
- 사람이 확인(ack)한 경보는 자동개입을 멈춘다(사람 우선).
- 모든 개입은 로그로 남는다: `heal_restart`(시도)·`heal_ok`(복구)·`heal_giveup`(포기).
